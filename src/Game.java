...
public class Game {
    ...
    public void start() {
        numberOfHero = tu.askInt("Combien de héros voulez-vous dans votre équipe (Entre 1 et 4) ?", 4); //pour checker que l'utilisateur répond une réponse valide
        System.out.println("Vous avez choisi " + numberOfHero + " héros dans votre équipe !");

        for (int i = 0; i < numberOfHero; i++){
            heroes.add(modComb.createHero(i));
        }
        showHeroTeam(); //Affiche le showteam dans la console

        isRunning = true;
        System.out.println("C'est parti pour le début de l'aventure !");
    ...
    public void showHeroTeam() {
        tu.separator(70, "-");
        tu.printTitle("VOICI TON EQUIPE DE PUISSANTS HEROS !");
        for (int i = 0; i < numberOfHero; i++) {
